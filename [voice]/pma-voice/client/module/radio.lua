			...
			--if GetConvarInt('voice_enableRadioAnim', 0) == 1 and not (GetConvarInt('voice_disableVehicleRadioAnim', 0) == 1 and IsPedInAnyVehicle(PlayerPedId(), false)) then
				--if not disableRadioAnim then
					...
					if not DoesEntityExist(radioProp) then
						radioProp = CreateObject(`v_serv_radio`, 1.0, 1.0, 1.0, true, true, false)
						AttachEntityToEntity(radioProp, PlayerPedId(), GetPedBoneIndex(PlayerPedId(), 60309), 0.0650, 0.0230, -0.0230, 180.0, 10.0, 240.0, true, false, false, false, 2, true)
					end
				--end
			--end
...
RegisterCommand('-radiotalk', function()
	if radioChannel > 0 or radioEnabled and radioPressed then
		...
		--if GetConvarInt('voice_enableRadioAnim', 0) == 1 then
			...
			if radioProp ~= 0 and DoesEntityExist(radioProp) then
